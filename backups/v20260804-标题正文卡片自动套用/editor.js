...
export const useEditorStore = defineStore('editor', () => {
  ...
  const componentHTML = (comp, text, stepNum) => {
    ...
    switch (comp.type) {
      ...
      case 'nextPreview': {
        // 下期预告：书卷式预告（三部分）
        // ① np-head 预告标题 ② np-body 预告内容 ③ np-follow 引导关注(单独换行+主题色+细分隔线)
        // 传入自定义文本(如 Markdown 解析的预告)时：按首个「：」拆标题/正文，「关注我」起为引导关注
        ...
        return `<div class="editable-block style-next-preview" data-style="nextPreview" style="margin-top:30px;padding:20px;background:${TF};border-radius:12px">` +
          `<table style="width:100%;border-collapse:collapse"><tr>` +
            ...
            `<td style="vertical-align:top;padding:4px 4px 4px 12px">` +
              ...
              `<div class="np-body" style="font-size:14.5px;color:#554a3d;line-height:1.8;margin-bottom:12px">${npBody}</div>` +
