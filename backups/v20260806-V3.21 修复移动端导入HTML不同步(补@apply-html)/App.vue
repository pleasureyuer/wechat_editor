    ...
    <main class="app-main-mobile">
      ...
      <div v-show="mobileTab === 'input'" class="mobile-panel">
        <LeftSidebar
          ...
          @apply-html="handleMobileApplyHTML"
          @load-full-sample="handleLoadFullSample"
        />
      </div>
...
import { useEditorStore, lightenToWhite, convertHtmlToWechatCompatible } from './stores/editor';
...
// 移动端导入 HTML：先切到编辑 Tab，等 DOM 更新后 Editor 可见时再注入
const handleMobileApplyHTML = async (htmlText) => {
  if (!htmlText?.trim()) return;
  mobileTab.value = 'edit';
  await nextTick(); // 等待 v-show 让 Editor 变为可见
  applyHTMLToEditor(htmlText);
};
...
// HTML 原样渲染：CSS 转内联、flex/grid 转 table 后注入主编辑器，预览/复制/推送自动同步
const applyHTMLToEditor = (htmlText) => {
  if (!htmlText?.trim()) return;
  const converted = convertHtmlToWechatCompatible(htmlText);
  if (editorRef.value) {
    editorRef.value.forceSaveBeforeChange();
