<template>
  <div class="right-panel">
    ...
    <!-- 组件默认文案 ── 引导关注/结尾等可配置 -->
    <div class="comp-text-panel">
      <button class="ct-toggle" @click="showCompText = !showCompText">
        <span>📝 组件默认文案</span>
        <span class="ct-arrow" :class="{ open: showCompText }">▸</span>
      </button>
      <Transition name="theme-panel-slide">
        <div v-if="showCompText" class="ct-body">
          <div class="ct-field">
            <label class="ct-label">下期预告 · 引导关注</label>
            <textarea v-model="npFollowText" class="ct-input" rows="2" @input="saveCompText" placeholder="例如：关注我，一起读懂改运的底层逻辑。"></textarea>
          </div>
          <div class="ct-field">
            <label class="ct-label">互动提问框 · 结尾</label>
            <textarea v-model="qaFooterText" class="ct-input" rows="2" @input="saveCompText" placeholder="例如：点赞 + 在看，夏日安康 ❤"></textarea>
          </div>
          <div class="ct-field">
            <label class="ct-label">留白金句 · 落款</label>
            <textarea v-model="zenFooterText" class="ct-input" rows="2" @input="saveCompText" placeholder="例如：晚安，我们明天见"></textarea>
          </div>
          <div class="ct-hint">留空则不显示该结尾；仅影响新插入/套用的组件，已插入的卡片可在正文直接修改。</div>
        </div>
      </Transition>
    </div>
...
// ── 组件默认文案（下期预告引导关注 / 互动提问框结尾）──
const COMP_TEXT_KEY = 'wechat_comp_text';
const showCompText = ref(false);
const npFollowText = ref('');
const qaFooterText = ref('');
const zenFooterText = ref('');

function loadCompText() {
  try {
    const map = JSON.parse(localStorage.getItem(COMP_TEXT_KEY) || '{}');
    npFollowText.value = map['nextPreview.follow'] ?? '关注我，一起读懂改运的底层逻辑。';
    qaFooterText.value = map['qaBox.footer'] ?? '点赞 + 在看，夏日安康 ❤';
    zenFooterText.value = map['zenQuote.footer'] ?? '晚安，我们明天见';
  } catch (e) {
    npFollowText.value = '关注我，一起读懂改运的底层逻辑。';
    qaFooterText.value = '点赞 + 在看，夏日安康 ❤';
    zenFooterText.value = '晚安，我们明天见';
  }
}

function saveCompText() {
  try {
    const map = JSON.parse(localStorage.getItem(COMP_TEXT_KEY) || '{}');
    map['nextPreview.follow'] = npFollowText.value;
    map['qaBox.footer'] = qaFooterText.value;
    map['zenQuote.footer'] = zenFooterText.value;
    localStorage.setItem(COMP_TEXT_KEY, JSON.stringify(map));
  } catch (e) {}
}
...
.ct-toggle:hover { background: #f0f2f5; }
.ct-arrow { transition: transform 0.2s; color: #999; font-size: 11px; }
.ct-arrow.open { transform: rotate(90deg); }
.ct-body { padding: 4px 14px 12px; }
.ct-field { margin-bottom: 10px; }
.ct-label { display: block; font-size: 11px; color: #888; margin-bottom: 4px; }
.ct-input {
...
.ct-input:focus { border-color: var(--theme-color, #0066ff); box-shadow: 0 0 0 2px rgba(0,102,255,0.1); }
.ct-hint { font-size: 11px; color: #aaa; line-height: 1.6; }
