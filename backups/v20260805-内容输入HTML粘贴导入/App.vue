...
import { useEditorStore, lightenToWhite, sanitizeHtmlForWechat } from './stores/editor';
...
// HTML 原样渲染（保留内联样式）：清洗后注入主编辑器，预览/复制/推送自动同步
const applyHTMLToEditor = (htmlText) => {
  if (!htmlText?.trim()) return;
  const cleaned = sanitizeHtmlForWechat(htmlText);
  if (editorRef.value) {
    editorRef.value.forceSaveBeforeChange();
    editorRef.value.setContent(cleaned);
  }
};
