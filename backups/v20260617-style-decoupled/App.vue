...
const componentHTML = (comp, text) => {
  ...
  switch (comp.type) {
    ...
    case 'gradientTitle': {
      const gt = txt || '渐变标题';
      return `<div class="editable-block style-gradient-title" data-style="gradientTitle"><h2 style="font-size:20px;font-weight:700;background:linear-gradient(90deg,${T},#a78bfa);-webkit-background-clip:text;-webkit-text-fill:transparent;margin:22px 0 12px;">${gt}</h2></div>`;
    }
    ...
    case 'leftLineTitle': {
      const lt = txt || '左竖线标题';
      return `<div class="editable-block style-left-line-title" data-style="leftLineTitle" style="margin:20px 0 12px"><h2 style="display:inline-block;border-left:4px solid ${T};padding-left:12px;font-size:17px;font-weight:700;line-height:1.4;color:#333;">${lt}</h2></div>`;
    }

    case 'rightLineTitle': {
      const rt = txt || '右竖线标题';
      return `<div class="editable-block style-right-line-title" data-style="rightLineTitle" style="text-align:right;margin:20px 0 12px"><h2 style="display:inline-block;text-align:right;border-right:4px solid ${T};padding-right:12px;font-size:17px;font-weight:700;line-height:1.4;color:#333;">${rt}</h2></div>`;
    }

    case 'centerLineTitle': {
      const ct = txt || '居中标题';
      return `<div class="editable-block style-center-title" data-style="centerLineTitle" style="margin:20px 0 12px;text-align:center"><h2 style="font-size:17px;font-weight:700;display:inline-block;border-bottom:2px solid ${T};padding-bottom:8px;color:#333;">${ct}</h2></div>`;
    }
    ...
    case 'stepTitle': {
      const st = txt || '步骤标题';
      return `<div class="editable-block style-step-title" data-style="stepTitle" style="display:flex;align-items:center;gap:10px;margin:16px 0 10px"><b style="color:${T};font-size:18px;font-weight:800">1</b><span style="font-weight:700;font-size:16px;color:#222">${st}</span></div>`;
    }
