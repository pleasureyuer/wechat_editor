...
export const useEditorStore = defineStore('editor', () => {
      ...
      switch(ds) {
        ...
        case 'simpleTable': {
          ...
          rows.forEach((row, i) => {
            ...
            cells.forEach(cell => {
              if (i === 0) {
                rowHTML += `<td style="padding:10px 12px;font-size:14px;font-weight:700;color:#fff;background-color:${T};border:1px solid ${T};">${cell.textContent.trim()}</td>`;
              } else {
                rowHTML += `<td style="padding:9px 12px;font-size:14px;color:#333;border:1px solid #e5e7eb;">${cell.innerHTML || cell.textContent.trim()}</td>`;
              }
            });
            if (i === 0) {
          ...
          out.push(`<section style="margin:16px 0;overflow:hidden;border-radius:6px;"><table style="width:100%;border-collapse:collapse;font-size:14px;">${tableHTML}</table></section>`);
          break;
        }
        ...
        case 'striTable': {
          ...
          rows.forEach((row, i) => {
            ...
            cells.forEach(cell => {
              if (i === 0) {
                rowHTML += `<td style="padding:10px 12px;font-size:14px;font-weight:700;color:${T};background-color:${TL};border-bottom:2px solid ${T};">${cell.textContent.trim()}</td>`;
              } else {
          ...
          out.push(`<section style="margin:16px 0;"><table style="width:100%;border-collapse:collapse;">${tableHTML}</table></section>`);
          break;
        }
        ...
        case 'borderTable': {
          ...
          rows.forEach((row, i) => {
            ...
            cells.forEach(cell => {
              if (i === 0) {
                rowHTML += `<td style="padding:10px 12px;font-size:14px;font-weight:700;color:${T};text-align:center;border:1px solid #ddd;background-color:${TL};">${cell.textContent.trim()}</td>`;
              } else {
                rowHTML += `<td style="padding:9px 12px;font-size:14px;color:#444;text-align:center;border:1px solid #ddd;">${cell.innerHTML || cell.textContent.trim()}</td>`;
              }
            });
            tableHTML += `<tr>${rowHTML}</tr>`;
          });
        ...
        case 'statCard': {
          ...
          out.push(`<section style="margin:16px 0;border-radius:8px;overflow:hidden;border:1px solid #eee;"><table style="width:100%;border-collapse:collapse;background-color:#fff;"><tr>${cellsHTML}</tr></table></section>`);
          break;
        }

        // ── 新增标题类组件 ──
        case 'dotLineTitle': {
          // 图1：圆点居中 + 框线标题（上方小圆点，下方横线，整体居中框式）
          const t = child.querySelector('.dl-text')?.innerHTML || child.textContent?.trim() || '圆点框线标题';
          out.push(`<table style="margin:18px auto;width:auto;"><tbody><tr><td style="text-align:center;padding:8px 20px;border:2px solid ${T};border-radius:6px;position:relative;"><span style="position:absolute;top:-7px;left:50%;transform:translateX(-50%);display:inline-block;width:7px;height:7px;background:${T};border-radius:50%;"></span><span style="font-size:17px;font-weight:700;color:#222;">${t}</span></td></tr></table>`);
          break;
        }
        case 'accentUnderline': {
          ...
          out.push(`<p style="margin:18px 0 6px;font-size:18px;font-weight:700;color:#222;display:inline-block;position:relative;padding-bottom:4px;"><span style="font-weight:700">${t}</span></p><p style="margin:0;padding:0;height:3px;background:${T};width:60px;border-radius:2px;font-size:1px;line-height:1px;"></p>`);
          break;
        }
        case 'solidBarTitle': {
        ...
        case 'diamondLineTitle': {
          // 图3：菱形装饰 + 延伸线标题（◇ 标题 ◇，左右横线）
          const t = child.querySelector('.dl-diamond-text')?.innerHTML || child.textContent?.trim() || '菱形延伸线标题';
          out.push(`<table width="100%" style="margin:20px 0;border-collapse:collapse;"><tbody><tr><td style="width:50%;padding-right:12px;border-bottom:1px solid ${TL}"></td><td style="white-space:nowrap;padding:0 8px;vertical-align:bottom;"><span style="color:${T};font-size:13px;margin:0 4px;">◇</span><span style="font-size:17px;font-weight:700;color:#222;">${t}</span><span style="color:${T};font-size:13px;margin:0 4px;">◇</span></td><td style="width:50%;padding-left:12px;border-bottom:1px solid ${TL}"></td></tr></tbody></table>`);
          break;
        }
        case 'circleStepBadge': {
          // 图4：圆形步骤徽章（橙色圆形数字+两侧横线）
          const numEl = child.querySelector('.csb-num');
          const numText = numEl ? (numEl.textContent?.trim() || '1') : '1';
          const outStr = `<table width="100%" style="margin:22px 0;border-collapse:collapse;"><tbody><tr><td style="width:45%;border-bottom:1px solid #ddd;"></td><td style="white-space:nowrap;padding:0 10px;vertical-align:bottom;text-align:center;"><span style="display:inline-block;width:40px;height:40px;line-height:38px;background:${T};color:#fff;font-size:18px;font-weight:800;border-radius:50%;text-align:center;vertical-align:middle;">${numText}</span></td><td style="width:45%;border-bottom:1px solid #ddd;"></td></tr></tbody></table>`;
          out.push(outStr);
          break;
        }
  ...
  const componentHTML = (comp, text, stepNum) => {
    ...
    switch (comp.type) {
      ...
      case 'dotLineTitle': {
        const t = txt || '圆点框线标题';
        return `<div class="editable-block style-dot-line-title" data-style="dotLineTitle" style="text-align:center;margin:18px auto;display:inline-block;position:relative;padding:10px 22px;border:2px solid ${T};border-radius:6px"><span style="position:absolute;top:-7px;left:50%;transform:translateX(-50%);display:inline-block;width:7px;height:7px;background:${T};border-radius:50%"></span><span class="dl-text" style="font-size:17px;font-weight:700;color:#222">${t}</span></div>`;
      }
      case 'accentUnderline': {
        const t = txt || '彩色下划线标题';
      ...
      case 'diamondLineTitle': {
        const t = txt || '菱形延伸线标题';
        return `<div class="editable-block style-diamond-line-title" data-style="diamondLineTitle" style="margin:20px 0"><table width="100%" style="border-collapse:collapse"><tbody><tr><td style="width:50%;padding-right:12px;border-bottom:1px solid ${TL}"></td><td style="white-space:nowrap;padding:0 8px;vertical-align:bottom"><span style="color:${T};font-size:13px;margin:0 4px">◇</span><span class="dl-diamond-text" style="font-size:17px;font-weight:700;color:#222">${t}</span><span style="color:${T};font-size:13px;margin:0 4px">◇</span></td><td style="width:50%;padding-left:12px;border-bottom:1px solid ${TL}"></td></tr></tbody></table></div>`;
      }
      case 'circleStepBadge': {
        const n = typeof stepNum === 'number' ? stepNum : (store?.getNextStepNum ? store.getNextStepNum() : 1);
        return `<div class="editable-block style-circle-step-badge" data-style="circleStepBadge" data-step-num="${n}" style="margin:22px 0"><table width="100%" style="border-collapse:collapse"><tbody><tr><td style="width:45%;border-bottom:1px solid #ddd"></td><td style="white-space:nowrap;padding:0 10px;text-align:center;vertical-align:bottom"><span style="display:inline-block;width:40px;height:40px;line-height:38px;background:${T};color:#fff;font-size:18px;font-weight:800;border-radius:50%;text-align:center"><span class="csb-num">${n}</span></span></td><td style="width:45%;border-bottom:1px solid #ddd"></td></tr></tbody></table></div>`;
      }
