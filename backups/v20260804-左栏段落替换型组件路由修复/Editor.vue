<template>
  <div class="editor-shell">
    ...
    <div class="toolbar">
      <div class="tb-group">
        <button class="tb-btn tb-btn-lg" title="撤销" @click="saveHistory(); undoHistory(); edRef?.focus({preventScroll:true}); sync()">↶ 撤销</button>
        <button class="tb-btn tb-btn-lg" title="重做" @click="saveHistory(); redoHistory(); edRef?.focus({preventScroll:true}); sync()">↷ 重做</button>
      </div>
    ...
    <!-- 浮动快捷工具栏 -->
    <div ref="floatBarRef" v-show="floatOn" class="float-bar" :style="floatPos" @mousedown.prevent>
      <!-- 移动端抽屉头部 -->
      ...
      <template v-if="floatMode === 'text'">
        ...
        <select class="fl-sel" :value="selFontSize" @mousedown.stop @change="applyFloatFontSize($event.target.value)">
      ...
      <template v-else-if="floatMode === 'block'">
        ...
        <div class="fl-group">
          ...
          <button v-for="c in interactiveComponents" :key="c.type" class="fl-btn fl-comp"
            @click="(TITLE_BODY_CARDS.includes(c.type) || BODY_ONLY_CARDS.includes(c.type)) ? applyBlockStyle(c.type) : emitInsert(c.type)" @contextmenu.prevent="emitInsert(c.type)">{{ c.icon }} {{ c.name }}</button>
        </div>
        <!-- 操作 -->
        <div class="fl-group">
...
import { TITLE_COMPONENTS, CARD_COMPONENTS, LIST_COMPONENTS, TABLE_COMPONENTS, DIVIDER_COMPONENTS, INTERACTIVE_COMPONENTS, TITLE_BODY_CARDS, BODY_ONLY_CARDS } from '../constants/catalog.js'
...
// 编辑区页背景层（所见即所得）：背景/边框随当前主题取色，padding 与产出一致
const pageBgStyle = computed(() => {
  const base = store.pageBgInline
  return (base ? base : '') + 'padding:28px 18px;box-sizing:border-box;'
})
...
// 让当前光标轻柔滚动到可视区域（仅在确实不在可视区时才滚动，避免过度跳转）
function scrollCaretIntoView() {
  ...
  const pad = 80
  // 已在可视区内则不动
  if (rect.top >= pad && rect.bottom <= vh - pad) return
  const target = rect.top < pad ? 'start' : 'end'
  // 优先用窗口级滚动（父容器无滚动时），或回退到 nearest
  try {
    if (target === 'start') window.scrollTo({ top: window.scrollY + rect.top - pad, behavior: 'auto' })
    else window.scrollTo({ top: window.scrollY + rect.bottom - vh + pad, behavior: 'auto' })
  } catch (e) {
...
function saveHistory() {
  ...
  const caret = getCaretOffset(edRef.value)
  const last = historyIndex.value >= 0 ? historyStack.value[historyIndex.value] : null
  // 跳过与上次相同的快照
  if (last && last.html === html) return
  // 丢弃当前位置之后的旧记录
  historyStack.value.splice(historyIndex.value + 1)
  historyStack.value.push({ html, caret })
  // 限制长度
...
function undoHistory() {
  ...
  edRef.value.innerHTML = snap.html
  store.editorContent = edRef.value.innerHTML
  restoreCaret(edRef.value, snap.caret)
  edRef.value.focus({ preventScroll: true })
  scrollCaretIntoView()
}
...
function onKey(e) {
  if (e.ctrlKey || e.metaKey) {
    ...
    if (k==='y') { e.preventDefault(); saveHistory(); redoHistory(); edRef.value?.focus({preventScroll:true}); sync(); return }
  }
  if (e.key==='Tab') {
    e.preventDefault()
...
function onSelect() {
  ...
  if (txt && edRef.value?.contains(sel.getRangeAt(0).commonAncestorContainer)) {
    ...
    if (isMobile.value) {
      ...
      const r = sel.getRangeAt(0)
      const rect = r.getBoundingClientRect()
      const barHeight = 52 // 文字模式工具栏约高
      const gap = 8
      const centerY = rect.top + rect.height / 2
      const screenMid = window.innerHeight / 2
      // 光标/选中区在屏幕中线以下 → 悬浮栏放上方；反之放下方
      let top = centerY > screenMid
        ? rect.top - barHeight - gap
        : rect.bottom + gap
      // 边界保护：防止极端情况下超出视口
      if (top < gap) top = rect.bottom + gap
      if (top + barHeight > window.innerHeight - gap) top = rect.top - barHeight - gap
      floatPos.value = {
        position: 'fixed',
        left: Math.max(8, Math.min(rect.left + rect.width/2 - 200, window.innerWidth - 540)) + 'px',
        top: Math.max(gap, top) + 'px',
        zIndex: 10000
      }
      adjustFloatBarPosition(rect, gap)
    }
    // 检测当前字号
    detectFontSize(sel)
    ...
    if (isMobile.value) {
      ...
      try {
        const r = sel.getRangeAt(0)
        const rect = r.getBoundingClientRect()
        const barHeight = 56 // 块模式工具栏约高
        const gap = 8
        const centerY = rect.top + rect.height / 2
        const screenMid = window.innerHeight / 2
        // 光标/选区在屏幕中线以下 → 悬浮栏放上方；反之放下方
        let top = centerY > screenMid
          ? rect.top - barHeight - gap
          : rect.bottom + gap
        // 边界保护
        ...
        floatPos.value = {
          position: 'fixed',
          left: Math.max(8, Math.min(rect.left, window.innerWidth - 770)) + 'px',
          top: Math.max(gap, top) + 'px',
          zIndex: 10000
        }
        adjustFloatBarPosition(rect, gap)
...
// 渲染后根据弹窗实测高度重新定位，避免估算高度不准导致压到光标行
function adjustFloatBarPosition(rect, gap = 8) {
  nextTick(() => {
    const el = floatBarRef.value
    if (!el || !floatOn.value) return
    ...
    floatPos.value.top = Math.max(gap, top) + 'px'
  })
}
...
function applyFloatFontSize(size) {
  ...
  // 记录当前选区，用于后续 font 替换范围判断
  const initialRange = range.cloneRange()

  // 用 execCommand 处理跨段落/跨元素选区（surroundContents 会失败）
  document.execCommand('fontSize', false, '7')

  // 把浏览器生成的 <font size="7"> 替换为带实际 px 的 span
  const fonts = []
  const walker = document.createTreeWalker(
    ...
    {
      acceptNode: (node) => {
        if (node === edRef.value) return NodeFilter.FILTER_SKIP
        if (node.tagName !== 'FONT' || node.getAttribute('size') !== '7') return NodeFilter.FILTER_SKIP
        const nodeRange = document.createRange()
        try { nodeRange.selectNode(node) } catch { return NodeFilter.FILTER_SKIP }
        const afterStart = nodeRange.compareBoundaryPoints(Range.START_TO_START, initialRange) >= 0
        const beforeEnd = nodeRange.compareBoundaryPoints(Range.END_TO_END, initialRange) <= 0
        const startsBefore = nodeRange.compareBoundaryPoints(Range.START_TO_START, initialRange) <= 0
        const endsAfter = nodeRange.compareBoundaryPoints(Range.END_TO_END, initialRange) >= 0
        const inside = afterStart && beforeEnd
        const partial = (startsBefore && endsAfter) && !inside
        if (inside || partial) return NodeFilter.FILTER_ACCEPT
        return NodeFilter.FILTER_SKIP
      }
    }
  )
  while (walker.nextNode()) fonts.push(walker.currentNode)
...
// 一键主题色：直接应用当前主题「强调色」到选中文字（金句/印章/重点标注）
function applyThemeColor() {
  wrapSelection('color', store.currentThemeAccent)
}
...
function applyBlockStyle(compType) {
  const sel = window.getSelection()
  // 移动端/外部点击（抽屉、浮动球）导致失焦时，恢复到上次保存的编辑器内选区
  if (savedRange && (!sel || sel.rangeCount === 0 || !edRef.value.contains(sel.anchorNode))) {
  ...
  // 支持「标题+正文」双段卡片的组件类型（来自 catalog.js）
  // 只填充正文内容区、保留组件默认开头/结尾的组件类型（来自 catalog.js）

  // 对标题+正文卡片，尝试提取标题和正文：
  // 1) 若选区跨两个块级元素 → 第一块为标题，第二块为正文
  // 2) 否则若下一段明显是正文 → 当前段为标题，下一段为正文
  let titleText = currentText
  let bodyText = ''
  let bodyHTML = ''
  let consumeNext = null

  if (TITLE_BODY_CARDS.includes(compType)) {
    const range = sel.rangeCount ? sel.getRangeAt(0) : null
    if (range && !range.collapsed) {
      const startBlock = getBlockAncestor(range.startContainer)
      const endBlock = getBlockAncestor(range.endContainer)
      // 仅当 start 与 end 是相邻的两个顶层块时才认为是「标题+正文」
      if (startBlock && endBlock && startBlock !== endBlock && startBlock.nextElementSibling === endBlock) {
        titleText = startBlock.textContent?.trim() || ''
        bodyText = endBlock.textContent?.trim() || ''
        targetEl = startBlock
        consumeNext = endBlock
      }
    }
    if (!consumeNext && targetEl.nextElementSibling) {
      const next = targetEl.nextElementSibling
      const nextText = next.textContent?.trim() || ''
      const blockTags = ['P','H1','H2','H3','H4','H5','H6','DIV','SECTION','BLOCKQUOTE']
      if (nextText && blockTags.includes(next.tagName?.toUpperCase()) && currentText.length <= 80 && nextText.length > currentText.length) {
        bodyText = nextText
        consumeNext = next
      }
    }
  ...
  // 只填充正文内容区：保留组件默认标题/结尾，中间用当前段落内容（含换行）填充
  if (BODY_ONLY_CARDS.includes(compType)) {
    bodyHTML = targetEl.innerHTML?.trim() || currentText
  }
  ...
  const compHTML = store.componentHTML(
    TITLE_BODY_CARDS.includes(compType)
      ? { type: compType, title: titleText, bodyText }
      : BODY_ONLY_CARDS.includes(compType)
        ? { type: compType, bodyText: bodyHTML }
        : { type: compType },
    currentText,
    stepNum
  )
  ...
  if (compNode && edRef.value) {
    ...
    targetEl.remove()
    if (consumeNext && consumeNext.parentElement) consumeNext.remove()
    onInput()
    // 把光标移到新组件后面
    const range = document.createRange()
...
// 找到节点所在的编辑器顶层块级元素（P/H1-H6/DIV/SECTION/BLOCKQUOTE）
function getBlockAncestor(node) {
  if (!node) return null
  let el = node.nodeType === 1 ? node : node.parentElement
  const blockTags = ['P','H1','H2','H3','H4','H5','H6','DIV','SECTION','BLOCKQUOTE']
  while (el && el !== edRef.value) {
    if (blockTags.includes(el.tagName?.toUpperCase())) return el
    el = el.parentElement
  }
  return null
}
...
function insertHTML(html) {
  if (!edRef.value) return
  const sel = window.getSelection()
  // 点击外部（浮动球/移动端抽屉项）导致失焦时，恢复到上次保存的编辑器内光标
  if (savedRange && (!sel || sel.rangeCount === 0 || !edRef.value.contains(sel.anchorNode))) {
    try { sel.removeAllRanges(); sel.addRange(savedRange) } catch (e) {}
  }
  edRef.value.focus()
...
defineExpose({ insertHTML, clear: clearAll, getContent: getHTML, setContent, run, applyThemeColor, clearBlockAtCursor, getScrollRatio, scrollToRatio, forceSaveBeforeChange, applyBlockStyle })
