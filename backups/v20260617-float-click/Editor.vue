  ...
  <div class="editor-shell">
    ...
    <div v-show="floatOn" class="float-bar" :style="floatPos">
      ...
      <template v-if="floatMode === 'text'">
        ...
        <button class="fl-btn fl-hl" @click="applyHighlight('yellow')" title="荧光黄">✦ 荧光</button>
        <button class="fl-btn fl-color-btn" @click.stop="showFloatColor=!showFloatColor;showFloatBg=false;showFloatHl=false" title="主题色">
          <span class="fl-dot" :style="{background:store.currentThemeColor}"></span> 主题色
        </button>
        <div v-if="showFloatColor" class="fl-pop">
          <input type="color" :value="store.currentThemeColor"
            @input="applyTextColor($event.target.value)" @change="showFloatColor=false" />
        </div>
        <button class="fl-btn fl-bg-btn" @click.stop="showFloatBg=!showFloatBg;showFloatColor=false;showFloatHl=false" title="背景色板">🎨 色板</button>
        <div v-if="showFloatBg" class="fl-pop fl-pop-grid">
          <button v-for="c in bgColors" :key="c" class="fl-swatch"
            :style="{background:c}" @click="applyBgColor(c);showFloatBg=false"></button>
        </div>
        <button class="fl-btn" :class="{on:st.b}" @click="run('bold')" title="加粗"><b>B 加粗</b></button>
        <button class="fl-btn" :class="{on:st.i}" @click="run('italic')" title="斜体"><i>I 斜体</i></button>
        <button class="fl-btn" :class="{on:st.u}" @click="run('underline')" title="下划线"><u>U 下划线</u></button>
        <button class="fl-btn fl-fontsize-btn" title="字号数值">{{ selDisplaySize }}</button>
        <div v-if="showFloatHl" class="fl-pop fl-pop-grid">
          <button class="fl-swatch hl-yellow" style="background:#fff9c4" @click="applyHighlight('#fff9c4');showFloatHl=false">黄</button>
          <button class="fl-swatch hl-green" style="background:#c8e6c9" @click="applyHighlight('#c8e6c9');showFloatHl=false">绿</button>
          <button class="fl-swatch hl-blue" style="background:#bbdefb" @click="applyHighlight('#bbdefb');showFloatHl=false">蓝</button>
          <button class="fl-swatch hl-pink" style="background:#f8bbd0" @click="applyHighlight('#f8bbd0');showFloatHl=false">粉</button>
        </div>
      </template>

      <!-- 模式2: 光标在段落 → 块样式：左键应用样式，右键插入新组件 -->
      <template v-else-if="floatMode === 'block'">
        <div class="fl-label">块:</div>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('numberTitle')" @contextmenu.prevent="emitInsert('numberTitle')">📋 编号</button>
        ...
        <button class="fl-btn fl-comp" @click="applyBlockStyle('infoBox')" @contextmenu.prevent="emitInsert('infoBox')">💭 对话</button>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('highlightBlock')" @contextmenu.prevent="emitInsert('highlightBlock')">🎨 色块</button>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('dotLine')" @contextmenu.prevent="emitInsert('dotLine')">● 圆点</button>
        ...
        <button class="fl-btn fl-comp" @click="applyBlockStyle('cardBox')" @contextmenu.prevent="emitInsert('cardBox')">📦 卡片</button>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('highlightBlock')" @contextmenu.prevent="emitInsert('highlightBlock')">💡 提示</button>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('disclaimer')" @contextmenu.prevent="emitInsert('disclaimer')">📌 结尾</button>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('disclaimer')" @contextmenu.prevent="emitInsert('disclaimer')">⚠️ 声明</button>
        <button class="fl-btn fl-comp" @click="applyBlockStyle('spacer')" @contextmenu.prevent="emitInsert('spacer')">📄 留白</button>
      </template>

      <!-- 模式3: 分割线 -->
      <template v-else-if="floatMode === 'divider'">
        <div class="fl-label">分割:</div>
        <button class="fl-btn fl-comp" @click="emitInsert('dividerDashed')">◇ 菱形</button>
        <button class="fl-btn fl-comp" @click="emitInsert('dividerSolid')">— 横线</button>
        <button class="fl-btn fl-comp" @click="emitInsert('dividerDot')">• 圆点</button>
        <button class="fl-btn fl-comp" @click="emitInsert('dividerThick')">━ 短横</button>
        <button class="fl-btn fl-clear" @click="clearCurrentLine">✕ 清除</button>
      </template>
    </div>
  </div>
</template>
...
const floatPos = ref({})
const floatMode = ref('text') // 'text' | 'block' | 'divider'
const showFloatColor = ref(false)
const showFloatBg = ref(false)
const showFloatHl = ref(false)
...
function onSelect() {
  ...
  // 关闭所有弹出
  showFloatColor.value = false
  showFloatBg.value = false
  showFloatHl.value = false
  ...
  if (txt && edRef.value?.contains(sel.getRangeAt(0).commonAncestorContainer)) {
    ...
    try {
      ...
      floatPos.value = {
        position: 'fixed',
        left: Math.min(rect.left, window.innerWidth-540) + 'px',
...
function applyHighlight(color) {
  wrapSelection('backgroundColor', color || '#fff9c4')
}
...
function applyBlockStyle(compType) {
  ...
  // 向上找到 editable-block 或者最近的块级元素（p/h1-h6/div）
  let targetEl = null
  let walk = el
  while (walk && walk !== edRef.value) {
    const tag = walk.tagName?.toUpperCase()
    if (walk.classList?.contains('editable-block') || ['P','H1','H2','H3','H4','H5','H6','DIV','SECTION','BLOCKQUOTE'].includes(tag)) {
      targetEl = walk
      break
    }
...
function onClick(e) {
  ...
  // 点击浮动栏外部关闭弹出
  if (!e.target.closest('.fl-pop') && !e.target.closest('.fl-color-btn')) {
    showFloatColor.value = false
  }
  if (!e.target.closest('.fl-pop') && !e.target.closest('.fl-bg-btn')) {
    showFloatBg.value = false
  }
}
...
/* 特殊按钮样式 */
.fl-hl { background: rgba(250,204,21,0.2)!important; color: #fbbf24!important; }
.fl-hl:hover { background: rgba(250,204,21,0.35)!important; }
.fl-color-btn, .fl-bg-btn, .fl-fontsize-btn {
...
.fl-comp { background: rgba(59,130,246,0.18)!important; color: #93c5fd!important; font-size: 11px!important; }
