...
import { useEditorStore, lightenToWhite, convertHtmlToWechatCompatible, rasterizeDecorativeBlocks } from './stores/editor';
...
const mobileFloatInsert = (type) => {
  // 段落替换型组件统一走 applyBlockStyle（标题+正文双段卡片 / 仅填充内容区）
  if (TITLE_BODY_CARDS.includes(type) || BODY_ONLY_CARDS.includes(type)) {
    editorRef.value?.applyBlockStyle(type)
  } else {
...
const themeCSSVars = computed(() => {
  ...
  return {
    ...
    '--theme-accent': t?.accent || t?.color || '#C45C5C',
    '--theme-second': t?.second || t?.color || '#8A9A5B',
    '--theme-text-main': t?.textMain || '#444038',
    '--theme-text-muted': t?.textMuted || '#887F6A',
    '--theme-bg-page': t?.bgPage || '#FFFFFF',
    '--theme-bg-card': t?.bgCard || '#FFFFFF',
    '--theme-border': t?.border || '#E6E2D3'
  };
});
...
const handleInsertComponent = (comp) => {
  ...
  // 段落替换型组件：把光标所在的段落转换为组件（标题+正文双段卡片 / 仅填充内容区）
  if (TITLE_BODY_CARDS.includes(compType) || BODY_ONLY_CARDS.includes(compType)) {
    editorRef.value?.applyBlockStyle(compType);
    return;
  }
...
// HTML 原样渲染（混合方案）：装饰块截图为图片 + 正文 CSS 转内联后注入主编辑器
const applyHTMLToEditor = async (htmlText) => {
  if (!htmlText?.trim()) return;

  // 临时进度提示（装饰块截图可能耗时数秒）
  const tip = document.createElement('div');
  tip.textContent = '正在优化渲染…';
  tip.style.cssText = 'position:fixed;left:50%;top:18px;transform:translateX(-50%);z-index:99999;background:#333;color:#fff;padding:8px 16px;border-radius:6px;font-size:14px;box-shadow:0 2px 8px rgba(0,0,0,.3);';
  document.body.appendChild(tip);

  try {
    // 1) 用【原始视觉】把装饰块（含渐变/阴影/伪元素）截图替换成 <img>
    let htmlForImport = htmlText;
    try {
      htmlForImport = await rasterizeDecorativeBlocks(htmlText, {
        onProgress: ({ done, total }) => {
          tip.textContent = total > 0 ? `正在优化渲染（${done}/${total}）…` : '正在优化渲染…';
        },
      });
    } catch (e) {
      htmlForImport = htmlText; // 截图失败则退回纯 HTML 转换
    }

    // 2) 整体转内联 + 微信兼容（装饰块已是 <img> 不受影响，正文保留可复制）
    const converted = convertHtmlToWechatCompatible(htmlForImport);
    if (editorRef.value) {
      editorRef.value.forceSaveBeforeChange();
...
const handleApplyMarkdown = (mdText) => {
  ...
  // 关键字→组件映射：按当前主题的配置匹配行首关键字，命中则渲染对应组件。
  // hideKeyword=true 时去掉关键字前缀；优先级高于内置特殊行逻辑。
  function tryKeywordMap(lineText) {
    const maps = editorStore.getKeywordMaps(editorStore.currentStylePreset) || [];
    if (!maps.length) return null;
    const stripped = lineText.replace(/^>\s?/, '').replace(/＊/g, '*').trim();
    for (const m of maps) {
      if (!m.enabled) continue;
      if (stripped.startsWith(m.keyword)) {
        const content = m.hideKeyword ? stripped.slice(m.keyword.length).trim() : stripped;
        return componentHTML({ type: m.component }, content);
      }
    }
    return null;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 关键字→组件映射（按主题配置，用户自定义优先）
    const kwHtml = tryKeywordMap(line);
    if (kwHtml) { output.push(kwHtml); continue; }
    ...
    if (line.startsWith('>') && editorStore.currentStylePreset !== 'yiming') {
      flushList();
      flushTable();
      const rawQ = line.replace(/^>\s?/, '').trim();
      const qInner = rawQ.replace(/^\*+/, '').replace(/\*+$/, '').trim();
      // 冷灰理性职场：下期预告 → 竖排书卷式（nextPreview），组件自带「下期预告」标签，去前缀避免重复
      if (/预告/.test(qInner) && editorStore.currentStylePreset === 'lengui') {
        const previewTitle = qInner.replace(/^(下一篇预告|下期预告|下篇预告)[：:\s·・\-–]*/, '').trim();
        const blockLines = [previewTitle];
        let j = i + 1;
        while (j < lines.length && lines[j].trim().startsWith('>')) {
          const n2 = lines[j].trim().replace(/^>\s?/, '').replace(/^\*+/, '').replace(/\*+$/, '').trim();
          if (n2) blockLines.push(n2);
          j++;
        }
        i = j - 1; // 跳过已合并的预告续行（for 循环会再 +1）
        output.push(componentHTML({ type: 'nextPreview' }, blockLines.join('<br>')));
        continue;
      }
      const quoteText = fmt(rawQ);
    ...
    if (editorStore.currentStylePreset === 'yiming') {
      ...
      while (k < lines.length) {
        ...
        // 关键字→组件映射（按主题配置，用户自定义优先于内置逻辑）
        const kwHtmlY = tryKeywordMap(norm);
        if (kwHtmlY) { out.push(kwHtmlY); k++; continue; }
        ...
        if (norm.startsWith('>')) {
          ...
          // 免责声明：无背景注释卡（顶部细线），不进预告块
          if (/（[^）]*出自小说|（[^）]*不涉及小说/.test(qInner)) {
            out.push(componentHTML({ type: 'noteNoBg' }, qInner));
            k++; continue;
          }
          if (/预告/.test(qInner)) {
            // 组件自带「下期预告」竖排标签，正文标题去掉「下一篇预告/下期预告/下篇预告」前缀，避免重复
            const previewTitle = qInner.replace(/^(下一篇预告|下期预告|下篇预告)[：:\s·・\-–]*/, '').trim();
            const block = [previewTitle];
            k++;
            ...
            // 易命术预设：预告用「竖排书卷式」(nextPreview)，与其它主题(卷色卡)区分
            if (editorStore.currentStylePreset === 'yiming') {
              out.push(componentHTML({ type: 'nextPreview' }, block.join('<br>')));
            } else {
        ...
        // ⑨-b 免责声明（非 > 行）：无背景注释卡（顶部细线）
        const discInner = norm.replace(/^\*+/, '').replace(/\*+$/, '').trim();
        if (/（[^）]*出自小说|（[^）]*不涉及小说/.test(discInner)) {
          out.push(componentHTML({ type: 'noteNoBg' }, discInner));
          k++; continue;
        }
...
const handlePushWechat = async (detail) => {
  ...
  const allBlocks = Array.from(tmp.querySelectorAll('p, div, section, .editable-block, li, span'));
  // 文章最开头部分：取前面若干个有效块（非空、非超长）
  const headBlocks = allBlocks
    .map(el => el.textContent.trim())
    .filter(raw => raw && raw.length <= 200)
    .slice(0, 3);

  let title = '';
  let digest = '';

  // 规则①（全局默认）：开头"文章标题：""文章简介："前缀段落 → 推送标题/简介，仅在文章最开头部分生效
  for (const raw of headBlocks) {
    if (!title && /^文章标题[：:]/.test(raw)) {
      title = raw.replace(/^文章标题[：:]/, '').trim();
      continue;
    }
    if (!digest && /^文章简介[：:]/.test(raw)) {
      digest = raw.replace(/^文章简介[：:]/, '').trim();
      continue;
    }
    if (title && digest) break;
  }

  // 规则②（兜底）：任意位置"标题：""摘要："前缀段落
  if (!title || !digest) {
  ...
  // 方法2（兜底）：从整个 innerText 正则提取"标题：""摘要："
  if (!title || !digest) {
  ...
  // 作者默认填「当前要推送的公众号名称」：统一从本地账号配置读取（PC/移动端一致），过滤未配置占位
  let authorName = '';
  try {
    const accs = JSON.parse(localStorage.getItem('wechat_accounts') || '[]');
    const curId = localStorage.getItem('wechat_current_account') || '';
    const cur = accs.find(a => a.id === curId) || accs[0];
    if (cur && cur.appId && cur.name) {
      const n = String(cur.name).trim();
      if (n && n !== '未配置' && n !== '已配置的公众号') authorName = n;
    }
  } catch (e) { /* 读取异常时作者留空，由用户手填 */ }
  syncAuthor.value = authorName;
  syncSourceUrl.value = '';
