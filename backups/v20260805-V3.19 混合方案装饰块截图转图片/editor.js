...
function applyPseudoElements(root, rules) {
  ...
  pseudoRules.forEach(({ baseSel, position, declarations }) => {
    ...
    targets.forEach(target => {
      ...
      if (contentDecl && contentDecl.v) {
        const raw = contentDecl.v.trim();
        if (raw !== "''" && raw !== '""' && raw !== '' && raw !== 'none') {
          // 优先处理 attr()（如 content: attr(data-num)）
          const attrMatch = raw.match(/^attr\(['"]?([^'"\)]+)['"]?\)$/i);
          if (attrMatch) {
            span.textContent = target.getAttribute(attrMatch[1].trim()) || '';
          } else if (!raw.startsWith('url(')) {
            // 去掉 CSS 字符串引号，并解码 \8086 这类 unicode escape
...
// ═══════════════════════════════════════════════════════════
//  混合方案：装饰块截图（HTML 保真 + 视觉装饰块转图片）
//  ─ 思路：用隐藏容器渲染【原始】HTML（含渐变/阴影/伪元素的真实视觉），
//    识别「微信不支持但视觉重要」的装饰块 → html2canvas 截图 → 上传微信 CDN →
//    替换成 <img>。其余正文仍走 convertHtmlToWechatCompatible 转内联，保留可复制。
// ═══════════════════════════════════════════════════════════
function isWechatIncompatible(el) {
  if (!el || el.nodeType !== 1) return false;
  const cs = getComputedStyle(el);
  if (/gradient/.test(cs.backgroundImage || '')) return true;
  if (cs.boxShadow && cs.boxShadow !== 'none' && cs.boxShadow !== '') return true;
  if (cs.textShadow && cs.textShadow !== 'none' && cs.textShadow !== '') return true;
  if (['flex', 'grid', 'inline-flex', 'inline-grid'].includes(cs.display)) return true;
  if (['fixed', 'absolute', 'sticky'].includes(cs.position)) return true;
  try {
    const b = getComputedStyle(el, '::before');
    if (b.content && b.content !== 'none' && b.content !== 'normal') return true;
    const a = getComputedStyle(el, '::after');
    if (a.content && a.content !== 'none' && a.content !== 'normal') return true;
  } catch {}
  return false;
}

function findDecorativeBlocks(root, limit = 20) {
  const blocks = [];
  const walk = (el) => {
    if (blocks.length >= limit || el.nodeType !== 1) return;
    if (isWechatIncompatible(el)) {
      blocks.push(el);
      return; // 整块截图，不递归子元素
    }
...
async function uploadBase64Image(base64) {
  try {
    const res = await fetch('/api/wechat/upload-base64-image', {
      method: 'POST',
...
// 把原始 HTML 中装饰块渲染成图片并替换，返回「装饰块已变 <img>」的 HTML 字符串。
// 注意：返回值仍可能含 <style>/class，调用方需再跑 convertHtmlToWechatCompatible 转内联。
export async function rasterizeDecorativeBlocks(rawHtml, { onProgress } = {}) {
  if (!rawHtml || !rawHtml.trim() || typeof document === 'undefined') return rawHtml;
  if (!rawHtml.includes('<')) return rawHtml;

  const container = document.createElement('div');
  container.style.cssText = 'position:fixed;left:-99999px;top:0;width:1080px;background:#fff;z-index:-1;overflow:hidden;';
  container.innerHTML = rawHtml;
  document.body.appendChild(container);

  try {
    // 先移除全屏级 fixed 装饰层（飘落叶片/背景纹理），避免被当成整屏装饰块截图
    Array.from(container.querySelectorAll('*')).forEach(el => {
      try { if (getComputedStyle(el).position === 'fixed') el.remove(); } catch {}
    });

    const blocks = findDecorativeBlocks(container, 20);
    if (onProgress) onProgress({ total: blocks.length, done: 0 });
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (onProgress) onProgress({ total: blocks.length, done: i + 1 });
    }
...
export const useEditorStore = defineStore('editor', () => {
  ...
  const buildWechatHTML = (editorHTML) => {
    ...
    for (const child of temp.children) {
      // 顶级图片（含「装饰块截图」生成的 <img>）：原样保留，避免被 innerHTML 吞掉
