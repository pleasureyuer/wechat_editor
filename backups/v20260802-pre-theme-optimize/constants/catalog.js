...
export const TITLE_COMPONENTS = [
  ...
  { type: 'cardTitle',       name: '卡片标题',       icon: '📋', preview: '<span style="display:inline-block;background:var(--theme-light,#e6f0ff);border:1px solid var(--theme-color,#0066ff);border-radius:6px;padding:6px 14px;font-weight:bold;color:var(--theme-color,#0066ff)">卡片标题</span>' },
...
export const CARD_COMPONENTS = [
  ...
  { type: 'nextPreview', name: '下期预告',   icon: '📜', desc: '结尾互动·书卷式下期预告', preview: '<div style="background:var(--theme-faint,#f8fbff);border-radius:8px;padding:6px;font-size:10px;display:flex;gap:4px"><span style="writing-mode:vertical-rl;color:var(--theme-color,#0066ff);font-size:9px;letter-spacing:2px;padding:0 2px">下期预告</span><div style="color:#555"><b style="color:var(--theme-color,#0066ff)">预告标题</b><br>简介文字…</div></div>' },
...
export const CARD_STYLE_OPTIONS = CARD_COMPONENTS
  .filter(c => !c.styleKind && c.type !== 'seriesOpening' && c.type !== 'topicSectionCard'
    && c.type !== 'qaBox' && c.type !== 'nextPreview' && c.type !== 'zenQuote')
  .map(c => ({ id: c.type, name: c.name, icon: c.icon }));
