  ...
  <div id="app" :data-theme="editorStore.currentTheme" :style="themeCSSVars">
    ...
    <main v-if="!isMobile" class="app-main">
      ...
      <!-- 中间：编辑区 -->
        <section class="editor-section">
          <Editor ref="editorRef" @update:content="onContentUpdate" @insert-component="onFloatInsertComponent" @scroll="onEditorScroll" @import-md="handleApplyMarkdown" />
        </section>
    ...
    <div v-else class="app-mobile">
      ...
      <div class="mobile-top-bar">
        <button class="mobile-back-btn" @click="activeMobileTab = 'edit'">←</button>
        <span class="mobile-title">{{ mobileTitle }}</span>
        <button class="mobile-copy-btn" @click="handleCopy">复制</button>
      </div>

      <!-- 移动端内容区 -->
      <div class="mobile-content">
        <!-- 编辑面板 -->
        <div v-show="activeMobileTab === 'edit'" class="mobile-panel">
          <Editor ref="mobileEditorRef" @update:content="onContentUpdate" @insert-component="onFloatInsertComponent" @import-md="handleApplyMarkdownMobile" />
        </div>

        <!-- 预览面板 -->
        <div v-show="activeMobileTab === 'preview'" class="mobile-panel">
          <RightPanel ref="mobilePreviewRef" @copy="handleCopy" @export="handleExport" />
        </div>

        <!-- 组件面板 -->
        <div v-show="activeMobileTab === 'components'" class="mobile-panel mobile-panel-scroll">
          <LeftSidebar
            @insert-component="handleInsertComponentMobile"
            @apply-markdown="handleApplyMarkdown"
            @load-full-sample="handleLoadFullSample"
          />
        </div>
        ...
        <!-- 主题面板 -->
        <div v-show="activeMobileTab === 'theme'" class="mobile-panel mobile-panel-scroll">
          <div class="mobile-theme-placeholder">主题设置（后期实现）</div>
        </div>
      </div>
...
const activeMobileTab = ref('edit');
...
const mobileTitle = computed(() => {
  const map = { edit: '编辑', preview: '预览', components: '组件', theme: '主题' };
  return map[activeMobileTab.value] || '编辑';
});

// 移动端插入组件：插入到移动端编辑器
const handleInsertComponentMobile = (comp) => {
  const compObj = typeof comp === 'string' ? { type: comp } : comp;
  const html = editorStore.componentHTML(compObj);
  mobileEditorRef.value?.insertHTML(html);
  // 插入后自动切回编辑面板
  activeMobileTab.value = 'edit';
};

// 移动端 MD 导入（路由到正确的编辑器 ref）
const handleApplyMarkdownMobile = (mdText) => {
...
const handleInsertComponent = (comp) => {
  ...
  const stepNum = (compType === 'stepTitle' || compType === 'circleStepBadge') ? store.getNextStepNum() : undefined;
...
const handleLoadFullSample = () => {
  ...
  const targetEditor = isMobile.value ? mobileEditorRef.value : editorRef.value;
  if (targetEditor) {
    targetEditor.setContent(allHTML);
  }
};
...
const handleApplyMarkdown = (mdText) => {
  ...
  // 根据当前设备使用正确的编辑器引用
  const targetEditor = isMobile.value ? mobileEditorRef.value : editorRef.value;
  targetEditor?.insertHTML(cleaned);
};
...
/* 编辑面板：重置内边距 */
.mobile-panel .editor-shell {
  padding: 0 !important;
}

/* 预览面板：重置样式 */
.mobile-panel .right-sidebar {
  border-left: none !important;
  padding: 0 !important;
}

/* 组件面板：重置样式 */
.mobile-panel .left-sidebar {
  width: 100% !important;
